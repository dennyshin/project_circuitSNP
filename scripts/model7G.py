...
for epoch in epochs:
	...
	if loss < min_val_loss:
		min_val_loss = loss
		torch.save(model, "trained_models/model7G.pt")
		opt_epoch = epoch
...
def plot_loss(training_loss, validation_loss):
	...
	ax.legend()
	fig.savefig('imgs/model7G.png')

plot_loss(train_loss, val_loss)

# save loss
with open('trained_models/model7G_opt.txt', 'w') as f:
	f.write("min val loss = %f\t at epoch %i" % (min_val_loss, opt_epoch))
np.savetxt('trained_models/model7G_trainloss.txt', train_loss, fmt='%f')
np.savetxt('trained_models/model7G_valloss.txt', val_loss, fmt='%f')
