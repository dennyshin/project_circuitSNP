...
def plot_loss(training_loss, validation_loss):
	...
	ax.legend()
	fig.savefig('imgs/model7I.png')

plot_loss(train_loss, val_loss)

# save loss
with open('trained_models/model7I_opt.txt', 'w') as f:
	f.write("min val loss = %f\t at epoch %i" % (min_val_loss, opt_epoch))
np.savetxt('trained_models/model7I_trainloss.txt', train_loss, fmt='%f')
np.savetxt('trained_models/model7I_valloss.txt', val_loss, fmt='%f')
